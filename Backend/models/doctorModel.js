...
const doctorSchema = new mongoose.Schema(
  {
    ...
    phone: {
      ...
      required: true,
      unique: true,
      maxLength: [10, "Phone number should have 10 digits only"],
    },
    email: {
      type: String,
      required: true,
    ...
    password: {
      ...
      select: false, // This ensures that the password is not included in queries by default
    },
    image: {
      type: String,
      required: true,
...
doctorSchema.pre("save", async function (next) {
  ...
  // and move to the next middleware or save operation
  this.password = await bycryp.hash(this.password, 10);
});

doctorSchema.methods.comparePassword = async function (enteredPassword) {
  return await bycryp.compare(enteredPassword, this.password);
};
...
doctorSchema.methods.getResetPasswordToken = function () {
  ...
  // Hash the reset token and set it to the doctor's resetPasswordToken field
